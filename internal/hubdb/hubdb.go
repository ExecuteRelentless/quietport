...
const circleCols = `id,slug,display_name,bucket_prefix,generation,quota_bytes,sync_mode,version_retention_days,excludes,bwlimit,s3_access_key,s3_secret_key,created_at`
...
func scanCircle(r interface{ Scan(...any) error }) (circleRow, error) {
	...
	err := r.Scan(&c.ID, &c.Slug, &c.DisplayName, &c.BucketPrefix, &c.Generation, &c.QuotaBytes, &c.SyncMode, &c.VersionRetentionDays, &ex, &c.BwLimit, &c.S3AccessKey, &c.S3SecretKey, &created)
	if errors.Is(err, sql.ErrNoRows) {
...
func (d *DB) CircleCreate(c model.Circle, s3Access, s3Secret string) (model.Circle, error) {
	...
	res, err := d.Exec(`INSERT INTO circle(slug,display_name,bucket_prefix,generation,quota_bytes,sync_mode,version_retention_days,excludes,bwlimit,s3_access_key,s3_secret_key,created_at) VALUES(?,?,?,1,?,?,?,?,?,?,?,?)`,
		c.Slug, c.DisplayName, c.BucketPrefix, c.QuotaBytes, c.SyncMode, c.VersionRetentionDays, string(ex), c.BwLimit, s3Access, s3Secret, now())
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE") {
...
func (d *DB) CircleUpdate(c model.Circle) error {
	ex, _ := json.Marshal(c.Excludes)
	_, err := d.Exec(`UPDATE circle SET display_name=?,quota_bytes=?,sync_mode=?,version_retention_days=?,excludes=?,bwlimit=? WHERE id=?`,
		c.DisplayName, c.QuotaBytes, c.SyncMode, c.VersionRetentionDays, string(ex), c.BwLimit, c.ID)
	return err
}
