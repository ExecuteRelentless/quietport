...
members see. A circle's folder on a member's computer is its own directory under `~/QPSync`, named after the display
name when the circle arrived there (`Shared`, or `Shared 2` when another folder on that computer has the name), and
it arrives with nothing in it (docs/adr/0021). A display name is always a name a directory can have (docs/adr/0020).
