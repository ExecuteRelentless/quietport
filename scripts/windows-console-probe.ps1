...
#
# PROBE-READY is the probe's own mark and CMDMARK is a second program's, written by cmd.exe. If either is missing
# from the capture then this console could not be read the way the test assumes, and the run says nothing either
# way: a capture that comes back empty for every program is a blind harness, not a program that printed nothing.
param([Parameter(Mandatory)][string]$Exe, [Parameter(Mandatory)][string]$Out)
Write-Host "PROBE-READY"
cmd.exe /c "echo CMDMARK"
...
[void]$sb.AppendLine("host=$($Host.Name) exit=$($p.ExitCode) cursorY=$($ui.CursorPosition.Y) buffer=$($ui.BufferSize.Width)x$($ui.BufferSize.Height) read rows 0..$end")
for ($y = 0; $y -le $end; $y++) {
  $line = New-Object Text.StringBuilder
  for ($x = 0; $x -lt $w; $x++) { [void]$line.Append($cells[$y, $x].Character) }
  [void]$sb.AppendLine("row $y | " + $line.ToString().TrimEnd())
}
